...
- [x] `Input`: курсор, выделение текста, вставка из буфера (Ctrl+V) — `InputEditor` + фокус/hit-test + WM_CHAR/WM_KEYDOWN.
- [x] `List` со скроллом (MVP: clip + scrollOffset, видимые элементы только в viewport; полная виртуализация — позже).
- [x] Критерий готовности: список из ~30 карточек PR скроллится без просадки FPS (`listRendersThirtyItemsUnderFrameBudget` < 16ms/frame).
